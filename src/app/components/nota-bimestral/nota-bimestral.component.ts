...
@Component({
  selector: 'app-nota-bimestral',
  templateUrl: './nota-bimestral.component.html',
  styleUrl: './nota-bimestral.component.css'
})
export class NotaBimestralComponent {
 notasbimestrales: FormGroup;
 ...
 constructor(
  ...
  this.notasbimestrales = new FormGroup({
  ...
  for (let i = 0; i < this.otherGradoAcademico.length; i++) {
    this.notasbimestrales.addControl(`inputGrado${i + 1}`, new FormControl(this.otherGradoAcademico[i].nombre));
  }
 }
 ...
 ngOnInit(): void {
  ...
  this.bimestreAcademicoService.getBimestre().subscribe(
    (data: BimestreAcademicoDTO[]) => {
      ...
      const selectedPeriodoId = this.notasbimestrales.get('selectPeriodo')?.value;
  ...
  this.notasbimestrales.get('selectPeriodo')?.valueChanges.subscribe(selectedPeriodoId => {
...
private setDefaultPeriodo(): void {
  ...
  if (periodoEncontrado) {
    this.notasbimestrales.patchValue({
...
EnviarDatos(grado: number) {
  const selectedPeriodo = this.notasbimestrales.get('selectPeriodo')?.value;
  const selectedBimestre = this.notasbimestrales.get('selectBimestre')?.value;
  const selectedAsignatura = this.notasbimestrales.get('selectAsignatura')?.value;
  ...
  this.route.navigate(['/ver-notas-bimestral']);
